(function () {
  ...
  // Build EMV from fields (WITHOUT CRC field 63)
  ...
  // CRC16-CCITT (0x1021, init 0xFFFF) over ASCII characters
  ...
  // Given base EMV and new amount (number), build new EMV with updated 54 + new CRC
  ...
  function wireUpLogic(root) {
    ...
    payBtn.addEventListener("click", () => {
      ...
      try {
        ...
        logDebug("Generated EMV:", finalEmv);

        currentQrCode = finalEmv;
        currentOrderId = null; // optionally set your own ID if ever needed
  ...
  // === ENSURE UI MOUNTED (USED BY INITIAL LOAD + MUTATIONS) ==============
  function ensurePaymentUIMounted() {
    ...
    if (sibling && sibling.id === ROOT_ID) {
      // Already mounted right under host
      return;
    }

    // If #am-payment-root exists somewhere else, remove it to avoid duplicates
  ...
  // === MOUNTING: PAGE LOAD + 3s DELAY + MUTATION OBSERVER ================
  ...
  function setupMutationObserver() {
    try {
      const observer = new MutationObserver(function () {
        // Anytime checkout content changes, re-ensure our UI exists
        ensurePaymentUIMounted();
      });
      ...
      logDebug("MutationObserver attached to document.body for re-mounting.");
    } catch (e) {
